...
export default function DashboardPage() {
  const cam = useCameraProcessor();
  const { state } = cam;
  const primary = state.primary;
  const on = cam.camState === "on";
  ...
  return (
    <div className="px-7 py-6">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div>
          ...
          <p className="text-sm text-txtdim">
            Webcam của bạn · phát hiện &amp; ước lượng nguy cơ sốt ngay trên trình duyệt
          </p>
        </div>
          ...
          <div className="flex items-center gap-2 rounded-full border border-line2 bg-surface px-3 py-1.5">
            <span
              className={cn(
                ...
                on ? "animate-pulse-dot bg-mint" : "bg-txtfaint",
              )}
            />
            <span className="font-mono text-xs text-txtdim">
              {on ? "ĐANG XỬ LÝ" : "CAMERA TẮT"}
            </span>
          </div>
          <span className="font-mono text-[10px] text-txtfaint">
            On-device · không gửi video lên server
          </span>
        </div>
      </div>
      ...
      <AlertPopup alertCount={state.alertCount} persons={state.persons} />
    </div>
  );
}
